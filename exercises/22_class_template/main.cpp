...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        int data[]{
            ...
            21, 22, 23, 24};
        */
        // 广播后对应位置的元素相加，只需要确定每个索引确定元素的实际位置即可，由于实际数据是一维存储的，根据上面的 data 可知，
        // 第一个维度 1 确定的是每 2 * 3 * 4 = 24 个元素为一组
        // 第二个维度 2 确定的元素值以每 3 * 4 = 12 个元素为一组，即上面 data 中上半部分(1-12)或下半部分(13-24)
        // 第三个维度 3 确定的元素值以每 4 个元素为一组，每组的 1-4、5-8、9-12
        // 因此通过索引 (a, b, c, d) 乘每个维度的对应的偏移量，即可确定在一维数组中的实际位置
        
        // 如果某个维度为 1 需要广播，只需要在计算实际位置时保持该维度的偏移量为 0，即不在该维度进行偏移
